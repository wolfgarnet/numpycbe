...
//Does this work? It's the absolute path, so -I info should not be necessary.
#include "/home/jens/spu_mult_shader.h"
#include   <pthread.h>
#include <libspe2.h>

//JMI:26/03/09 - Includes needed because of the new way of calling the dispatcher
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#include "/home/jens/files_with_comments/pputest/libppu.c"
#include "/home/jens/files_with_comments/pputest/binreader.c"

#define MAX_SPE_THREADS 6 //Watch out! This is present in more than one place!!!
...
unsigned int *multShaderPointer;

typedef struct PyArrayObject2 {
  //JMI - just elide this for now. PyObject_HEAD
  //char *data;             /* pointer to raw data buffer */
  //int nd;                 /* number of dimensions, also called ndim */
  /*npy_intp*///int *dimensions;       /* size in each dimension */
  /*npy_intp*///int *strides;          /* bytes to jump to get to the
  ...
  //JMI: Added members to support new memory layout
  char **blockData;
...
}PyArrayObject2 __attribute__((aligned (16)));


//////////////////////////////////////////////////////////////////////////////////////////////////////////////

//int MULT2( PyArrayObject2 *pyobj1, PyArrayObject2 *pyobj2, unsigned int shadersize, unsigned int *shader )
//{
//  printf("testeslestetest\n");
//}
...
int MULT( PyArrayObject2 *pyobj1, PyArrayObject2 *pyobj2, PyArrayObject2 * resObj, unsigned int shadersize, unsigned int *shader )
{
  printf("MULT ON THE PPE IS CALLED!\n");
  //We need a result - for now we just create a result matrix here
  PyArrayObject2 res;

  //We also need to decrement the SPE id by one, when we receive it.
  ...
  float *results[4];
  printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!IN MULT NO OF SPE's IS: %d\n",speThreads);
  ...
  for ( i = 0 ; i < speThreads ; i++ )
    {
      ...
      printf("TO THE DISP. THE ID: %d, has just been sent\n",&state);
  ...
  */
  ...
  for ( i = 0 ; i < speThreads ; i++ )
    {
      ...
      //spe_in_mbox_write ( speData[i].spe_ctx, &r, 1, SPE_MBOX_ALL_BLOCKING );
      printf("TO THE DISP. THE SEED: %d, has just been sent\n",&state);
      printf("This is the actual value: %d\n",state);
    }
  ...
  for ( i = 0 ; i < speThreads ; i++ )
    {
      ...
      spe_in_mbox_write ( speData[i].spe_ctx, &state, 1, SPE_MBOX_ALL_BLOCKING );
      //spe_in_mbox_write ( speData[i].spe_ctx, &r, 1, SPE_MBOX_ALL_BLOCKING );
      printf("TO THE DISP. THE STATE: %d, has just been sent\n",&state);
      printf("This is the actual value: %d\n",state);
    }


  printf("Waiting for the SPE's\n");//ALSO, check that the structure used is correct!
  // Waiting for SPEs!
  ...
  while( checked < speThreads )
	...
	{
	  //JMI: We don't want to read anything, since we are actually running the MULT instead of SDOT.
	  //INSTEAD START BY CHECKING THE RESULT ON THE DISPATCHER I GUESS...
...
array_multiply_custom_new(PyArrayObject2 *m1, PyArrayObject2 *m2)
{
  printf("Inside new mult\n");
  //This is the new mult-entrance - it must call MULT to use the new dispatcher.
  //Also, it must get the shader from file and supply the pointer to MULT.
  //int MULT( PyArrayObject *pyobj1, PyArrayObject *pyobj2, PyArrayObject * resObj, unsigned int shadersize, unsigned int *shader )

  //load the multshader using the binreader
  //unsigned int binreader( char *binfile, unsigned int **bin )
  unsigned int **pointerToShader = _malloc_align(sizeof(unsigned int**),7);
  //printf("111 *pointerToShader: %d\n",*pointerToShader);
  printf("test5\n");
  unsigned int lengthOfShader = binreader("multshader.bin", pointerToShader);
  printf("Returned from lengthOfShader: %d\n",lengthOfShader);
  PyArrayObject2 *m3 = _malloc_align(sizeof(PyArrayObject2),7);
  //multShaderPointer = *pointerToShader;
  printf("222 *pointerToShader: %d\n",*pointerToShader);
  printf("111111111111111111111111111 before calling MULT inside array_multiply_custom_new\n");
  MULT(m1, m2, m3, lengthOfShader, *pointerToShader);
}

static void
GenericShaderSwitcher(unsigned int op,PyArrayObject2 *m1, PyObject *m2)
{
  printf("In the generic function\n");
  //This function collects all calls from add,subtract, multiply and so on...
  printf("Op is: %d\n",op);
  ...
  case 2:
    //mult
    printf("SHADER SWITCHER........................................\n");
    array_multiply_custom_new(m1, m2);
    //main_from_PPUTEST();
    break;
  default:
    break;
...
uint32_t main2[] __attribute__((aligned(128))) = 
{
  0x12000021, // hbrr c64 <main+0x84>,0 
  0x24004080, // stqd $0,16($1) 
  0x24ffc0d0, // stqd $80,-16($1) 
  0x24ff80d1, // stqd $81,-32($1) 
  0x408000d1, // il $81,1 
  0x24ff40d2, // stqd $82,-48($1) 
  0x24ff00d3, // stqd $83,-64($1) 
  0x24fec0d4, // stqd $84,-80($1) 
  0x4020007f, // nop $127 
  0x24fe80d5, // stqd $85,-96($1) 
  0x040001d5, // ori $85,$3,0 
  0x24fe40d6, // stqd $86,-112($1) 
  0x42000003, // ila $3,0 
  0x24fe00d7, // stqd $87,-128($1) 
  0x24fdc0d8, // stqd $88,-144($1) 
  0x24fd80d9, // stqd $89,-160($1) 
  0x24fd40da, // stqd $90,-176($1) 
  0x35900000, // hbrp c24 <main+0x44>,$0 
  0x24fd00db, // stqd $91,-192($1) 
  0x24fcc0dc, // stqd $92,-208($1) 
  0x24fc80dd, // stqd $93,-224($1) 
  0x24fc40de, // stqd $94,-240($1) 
  0x24fc00df, // stqd $95,-256($1) 
  0x24fbc0e0, // stqd $96,-272($1) 
  0x24fb80e1, // stqd $97,-288($1) 
  0x24fb40e2, // stqd $98,-304($1) 
  0x24fb00e3, // stqd $99,-320($1) 
  0x24fac0e4, // stqd $100,-336($1) 
  0x24fa80e5, // stqd $101,-352($1) 
  0x24fa40e6, // stqd $102,-368($1) 
  0x4020007f, // nop $127 
  0x24f9c081, // stqd $1,-400($1) 
  0x1c9c0081, // ai $1,$1,-400 
  0x33000000, // brsl $0,0 
  0x33000000, // brsl $0,0 
  0x04000184, // ori $4,$3,0 
  0x42000003, // ila $3,0 
  0x33000000, // brsl $0,0 
  0x4020007f, // nop $127 
  0x12000009, // hbrr ca0 <main+0xc0>,0 
  0x408002a0, // il $32,5 
  0x3380001f, // lqr $31,0 
  0x4020007f, // nop $127 
  0x3ec100a1, // cwd $33,4($1) 
  0x42000004, // ila $4,0 
  0x42000003, // ila $3,0 
  0xb3c7d021, // shufb $30,$32,$31,$33 
  0x2380001e, // stqr $30,0 
  0x33000000, // brsl $0,0 
  0x21a00b51, // wrch $ch22,$81 
  0x42000004, // ila $4,0 
  0x42000003, // ila $3,0 
  0x04002a85, // ori $5,$85,0 
  0x33000000, // brsl $0,0 
  0x4200001d, // ila $29,0 
  0x1200000e, // hbrr cf4 <main+0x114>,0 
  0x21a0081d, // wrch $ch16,$29 
  0x21a00955, // wrch $ch18,$85 
  0x40801052, // il $82,32 # 20 
  0x21a009d2, // wrch $ch19,$82 
  0x21a00a51, // wrch $ch20,$81 
  0x40802050, // il $80,64 # 40 
  0x21a00ad0, // wrch $ch21,$80 
  0x40800153, // il $83,2 
  0x21a00b53, // wrch $ch22,$83 
  0x21a00bd3, // wrch $ch23,$83 
  ...
  0x42000003, // ila $3,0 
  0x3380001c, // lqr $28,0 
  0x3f820e04, // rotqbyi $4,$28,8 
  0x33000000, // brsl $0,0 
  0x42000003, // ila $3,0 
  0x3380001b, // lqr $27,0 
  0x3f830d84, // rotqbyi $4,$27,12 
  0x33000000, // brsl $0,0 
  0x40801004, // il $4,32 # 20 
  0x00200000, // lnop 
  0x42000003, // ila $3,0 
  0x33000000, // brsl $0,0 
  0x21a00b51, // wrch $ch22,$81 
  ...
  0x33000000, // brsl $0,0 
  0x42000054, // ila $84,0 
  0x1200000a, // hbrr d64 <main+0x184>,0 
  0x3380001a, // lqr $26,0 
  0x04002a19, // ori $25,$84,0 
  0x3f820d18, // rotqbyi $24,$26,8 
  0x21a00819, // wrch $ch16,$25 
  0x21a00918, // wrch $ch18,$24 
  0x21a009d2, // wrch $ch19,$82 
  0x21a00a51, // wrch $ch20,$81 
  0x21a00ad0, // wrch $ch21,$80 
  0x42000003, // ila $3,0 
  0x33000000, // brsl $0,0 
  0x42000016, // ila $22,0 
  0x1200000a, // hbrr d94 <main+0x1b4>,0 
  0x33800017, // lqr $23,0 
  0x3f830b95, // rotqbyi $21,$23,12 
  0x21a00816, // wrch $ch16,$22 
  0x21a00915, // wrch $ch18,$21 
  0x21a009d2, // wrch $ch19,$82 
  0x21a00a51, // wrch $ch20,$81 
  0x21a00ad0, // wrch $ch21,$80 
  0x00200000, // lnop 
  ...
  0x33000000, // brsl $0,0 
  0x4020007f, // nop $127 
  0x1200000a, // hbrr dc4 <main+0x1e4>,0 
  0x42000014, // ila $20,0 
  0x33800013, // lqr $19,0 
...
{
  printf("arrayobject::_array_copy_into........................................\n");
    int swap;
    void (*myfunc)(char *, intp, char *, intp, intp, int);
    int simple;
    int same;
...
slice_GetIndices(PySliceObject *r, intp length,
                 ...
                 intp *slicelength)
{
  printf("arrayobject::slice_GetIndices\n");
  printf("r: %d\n",r);
  printf("length: %d\n",length);
  printf("start: %d\n",*start);
  printf("stop: %d\n",*stop);
  ...
  printf("*(r->step): %d\n",((unsigned int*)(r->step))[2]);
  printf("*(r->start): %d\n",((unsigned int*)(r->start))[2]);
  printf("*(r->stop): %d\n",((unsigned int*)(r->stop))[2]);
  
  printf("*(r->step): %d\n",((unsigned int*)(r->step))[0]);
  printf("*(r->start): %d\n",((unsigned int*)(r->start))[0]);
  printf("*(r->stop): %d\n",((unsigned int*)(r->stop))[0]);

  printf("*(r->step): %d\n",((unsigned int*)(r->step))[1]);
  printf("*(r->start): %d\n",((unsigned int*)(r->start))[1]);
  printf("*(r->stop): %d\n",((unsigned int*)(r->stop))[1]);

  printf("*(r->step): %d\n",((unsigned int*)(r->step))[3]);
  printf("*(r->start): %d\n",((unsigned int*)(r->start))[3]);
  printf("*(r->stop): %d\n",((unsigned int*)(r->stop))[3]);

  printf("*(r->step): %d\n",((unsigned int*)(r->step))[4]);
  printf("*(r->start): %d\n",((unsigned int*)(r->start))[4]);
  printf("*(r->stop): %d\n",((unsigned int*)(r->stop))[4]);
    ...
    if (r->step == Py_None) {
      printf("r->step == Py_None\n");
      //printf((char*)((char*)r->step)[8]);
      //printf((char*)((char*)r->step)[12]);


        *step = 1;
    } else {
      if (!slice_coerce_index(r->step, step)){printf("!slice_coerce_index(r->step, step)"); return -1;}
        if (*step == 0) {
            PyErr_SetString(PyExc_ValueError,
    ...
    //printing stuff out again to see the changes...
    printf("arrayobject::slice_GetIndices\n");
    printf("r: %d\n",r);
    printf("length: %d\n",length);
    printf("start: %d\n",*start);
...
{
  printf("arrayobject::parse_subsindex\n");
  //printf("*op: %d\n",*((unsigned int*)op[2]));
  printf("*step_size: %d\n",*step_size);
  printf("*n_steps: %d\n",*n_steps);
  printf("max: %d\n",max);
    ...
    printf("Printing out stuff in the end of par_subindex\n");
    printf("arrayobject::parse_subsindex\n");
    //printf("*op: %d\n",*((unsigned int*)op[2]));
    printf("*step_size: %d\n",*step_size);
    printf("*n_steps: %d\n",*n_steps);
...
{
  printf("arrayobject.c46\n");

  //JMI: strides and offest_ptr at the end of this function can
  //be used to determine the row of the matrix that should be sliced.


  printf("arrayobjecy::parse_index\n");
  printf("*dimensions : %d\n",*dimensions);
  printf("dimensions : %d\n",dimensions);
  printf("*strides: %d\n",*strides);
  printf("*offset_ptr: %d\n",*offset_ptr);
  ...
  printf("self->data[0]: %d\n",self->data[0]);
    ...
    *offset_ptr = offset;
    ...
    printf("Print stuff at the end of parse_index\n");
    printf("arrayobjecy::parse_index\n");
    printf("*dimensions : %d\n",*dimensions);
    printf("dimensions : %d\n",dimensions);
    printf("*strides: %d\n",*strides);
...
array_subscript_simple(PyArrayObject *self, PyObject *op)
{
  printf("arrayobject.c51\n");
    ...
    printf("object has these values:\n");
    printf("m1->data[0]: %d\n",((unsigned int*)((PyArrayObject*)other)->data)[0]);
    printf("m1->data[0]: %d\n",((unsigned int*)((PyArrayObject*)other)->data)[1]);
    printf("m1->dimensions[0]: %d\n",((PyArrayObject*)other)->dimensions[0]);
    printf("m1->nd: %d\n",((PyArrayObject*)other)->nd);
    printf("Done printing object...\n");

    

    //return (PyObject *)other;
    self->slice_type = 0;//Or whatever it needs to be!
    return (PyObject *)self;
}
...
{
  printf("-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-\n");
  printf("Binary function...\n");
  printf("OP[0] and OP[1] is: %d, %d\n",op[0],op[1]);
  //printf("OP->data is: %d",op->data);
  //printf("OP->data[0] is: %d",op->data[0]);
  printf("-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-7-\n");
    if (op == NULL) {
        Py_INCREF(Py_NotImplemented);
        return Py_NotImplemented;
    }
    printf("Before Returning from GenericBinaryFunction\n");
...
{
  printf("arrayobject.c65callfunction\n");
    if (op == NULL) {
        Py_INCREF(Py_NotImplemented);
        return Py_NotImplemented;
    }
...
{
  printf("arrayobject.c67callfunction\n");
    if (op == NULL) {
        Py_INCREF(Py_NotImplemented);
        return Py_NotImplemented;
    }
...
static __inline void _free_align2(void *ptr)
{
  printf("arrayobject.c69\n");
  void * real;
...
//Must be made dynamic, or course.
unsigned int R1_1[16]  __attribute__((aligned (16)));
unsigned int R1_2[16] __attribute__((aligned (16)));
unsigned int R2_1[16] __attribute__((aligned (16)));
unsigned int R2_2[16] __attribute__((aligned (16)));
unsigned int numberOfBlocks = 4;
unsigned int newRowSize = 16;//JMI:13/03/09 aendret fra 4 til 16.Assuming we measure the row-size in bytes.
unsigned int newColSize = 4;//JMI:13/03/09 aendret fra 2 til 4
unsigned int blockSize = 64;

PyArrayObject2 pyObject1;
PyArrayObject2 pyObject2;
PyArrayObject2 resultObject;


void initializeResultMatrix()
{
  printf("arrayobject.c70\n");
  //Assumes that the example matrices have already been sat
  numberOfBlocks = 4;
  newRowSize = 16;//JMI:13/03/09 - changed from 2 to 16
  newColSize = 4;//JMI:13/03/09 - changed from 2 to 4
  
  resultObject.numberOfBlocks = 4;
  resultObject.numberOfBlocksXDim = 2;
  resultObject.numberOfBlocksYDim = 2;

  resultObject.blockData = (char**)_malloc_align(sizeof(char*)*numberOfBlocks,4);
  resultObject.blockRowSize = _malloc_align(sizeof(unsigned int)*numberOfBlocks,4);
  resultObject.blockColSize = _malloc_align(sizeof(unsigned int)*numberOfBlocks,4);
  resultObject.blockSizes = _malloc_align(sizeof(unsigned int)*numberOfBlocks,4);

  unsigned int i = 0;
  for(i = 0;i<numberOfBlocks;i++)
    {
      //pyObject1.blockData[i] = (char*)_malloc_align(blockSize,4);
      resultObject.blockRowSize[i] = newRowSize;
      resultObject.blockColSize[i] = newColSize;
      resultObject.blockSizes[i] = blockSize;
    }
  //Set pointers to mem
  resultObject.blockData[0] = (char*)R1_1;
  resultObject.blockData[1] = (char*)R2_1;
  resultObject.blockData[2] = (char*)R1_2;
  resultObject.blockData[3] = (char*)R2_2;
  printf("!!!!!!!!!!!!!!!Done setting metadata on PyObject1.........\n");

  //Should we also set stuff such as reference count?

}

typedef struct TEST_STRUCT {
  //This struct is used by an SPE to see how much
  //work there is for it, and gives it
  //a way of seeing it's total amount of work, so
  //it knows when it is finishied with.

  unsigned int intA;
  unsigned int intB;
  unsigned int padding[2];

}test_struct __attribute__((aligned (16)));

/*
typedef struct SPE_JOB_INFO {
  //This struct is used by an SPE to see how much
  //work there is for it, and gives it
  //a way of seeing it's total amount of work, so
  //it knows when it is finishied with.
  ...
  unsigned int SPE_id;
  unsigned int howManySPEsAreUsed;
  PyArrayObject2 *pyObject1;
  PyArrayObject2 *pyObject2;
  PyArrayObject2 *resultObject;
  unsigned int functionPointer;
  unsigned int shaderSize;
  unsigned int printFunctionPointer;
  unsigned int EA;//For testing dma transfers initiated from shader
  unsigned int padding[3];

}SPE_JOB_INFO __attribute__((aligned (16)));
*/
typedef struct SPE_JOB_INFO {
unsigned int SPE_id;
unsigned int howManySPEsAreUsed;
PyArrayObject *pyObject1;
PyArrayObject *pyObject2;
PyArrayObject *resultObject;
unsigned int functionPointer;
unsigned int shaderSize;
unsigned int printFunctionPointer;
unsigned int EA;//For testing dma transfers initiated from shader
unsigned int pyObject1Pointer;
unsigned int pyObject2Pointer;//LS
unsigned int pyObjectResultPointer;//LS
}SPE_JOB_INFO __attribute__((aligned (16)));

//One job for each SPE that is used.
SPE_JOB_INFO speJobs[4];
SPE_JOB_INFO *pointersToSpeJobs[4];
unsigned int y;

typedef struct pThreadStruct{
  spe_context_ptr_t *contextPtr;
  unsigned int spe_id;
}pThreadStruct;

//SET THE NECESSARRY DATA: LIKE WHAT THE FUNCTION DivideJobsBetweenSPEs does and more...
//HERE

//Are alignment needed here?
unsigned int M1B1_1[16] __attribute__((aligned (16)));
unsigned int M1B2_1[16] __attribute__((aligned (16)));
unsigned int M1B1_2[16] __attribute__((aligned (16)));
unsigned int M1B2_2[16] __attribute__((aligned (16)));
unsigned int M1B3_1[16] __attribute__((aligned (16)));
unsigned int M1B3_2[16] __attribute__((aligned (16)));
unsigned int M2B1_1[16] __attribute__((aligned (16)));
unsigned int M2B2_1[16] __attribute__((aligned (16)));
unsigned int M2B1_2[16] __attribute__((aligned (16)));
unsigned int M2B2_2[16] __attribute__((aligned (16)));
unsigned int M2B1_3[16] __attribute__((aligned (16)));
unsigned int M2B2_3[16] __attribute__((aligned (16)));
//unsigned int R1_1[16] __attribute__((aligned (16)));
//unsigned int R1_2[16] __attribute__((aligned (16)));
//unsigned int R2_1[16] __attribute__((aligned (16)));
//unsigned int R2_2[16] __attribute__((aligned (16)));

unsigned int *testPointer = M1B1_1;

//PyArrayObject2 pyObject1;
//PyArrayObject2 pyObject2;
//PyArrayObject2 resultObject;

void fillExampleMatrices()
{
  printf("fillExampleMatrices begin\n");
  //Remember to consider endianess
  unsigned int count;
  ...
  //unsigned int *blockColSize;
  //unsigned int numberOfBlocks = 4;
  //unsigned int blockSize = 64;//In bytes. Is this correct?
  unsigned int newRowSize = 16;//JMI: 13/03/09 - Given in bytes
  //unsigned int newColSize = 4;
  //Still missing some metadata - these must be malloc'ed.
  pyObject1.numberOfBlocks = 6;
  pyObject1.numberOfBlocksXDim = 3;
  pyObject1.numberOfBlocksYDim = 2;

  pyObject1.blockData = (char**)_malloc_align(sizeof(char*)*pyObject1.numberOfBlocks,4);
  pyObject1.blockRowSize = _malloc_align(sizeof(unsigned int)*pyObject1.numberOfBlocks,4);
  pyObject1.blockColSize = _malloc_align(sizeof(unsigned int)*pyObject1.numberOfBlocks,4);
  pyObject1.blockSizes = _malloc_align(sizeof(unsigned int)*pyObject1.numberOfBlocks,4);

  printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
  printf("pyObject1.blockData is: %d\n",pyObject1.blockData);
  printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");

  unsigned int i = 0;
  for(i = 0;i<pyObject1.numberOfBlocks;i++)
    {
      //pyObject1.blockData[i] = (char*)_malloc_align(blockSize,4);
      pyObject1.blockRowSize[i] = newRowSize;
      pyObject1.blockColSize[i] = newColSize;
      pyObject1.blockSizes[i] = blockSize;
    }
  printf("!!!!!!!!!!!!!!!Done setting metadata on PyObject1.........\n");
  printf("The address of the block[0] on PyObject1 is: %d\n",M1B1_1);
  printf("The address of block[0] seen though pyObject1 is: %d\n",pyObject1.blockData[0]);
  printf("The address of pyObject1 is: %d\n",&pyObject1);
  
  pyObject2.numberOfBlocks = 6;
  pyObject2.numberOfBlocksXDim = 2;
  pyObject2.numberOfBlocksYDim = 3;

  pyObject2.blockData = (char**)_malloc_align(sizeof(char*)*pyObject2.numberOfBlocks,4);
  pyObject2.blockRowSize = _malloc_align(sizeof(unsigned int)*pyObject2.numberOfBlocks,4);
  pyObject2.blockColSize = _malloc_align(sizeof(unsigned int)*pyObject2.numberOfBlocks,4);
  pyObject2.blockSizes = _malloc_align(sizeof(unsigned int)*pyObject2.numberOfBlocks,4);

  for(i = 0;i<pyObject2.numberOfBlocks;i++)
    {
      //pyObject2.blockData[i] = (char*)_malloc_align(blockSize,4);
      pyObject2.blockRowSize[i] = newRowSize;
      pyObject2.blockColSize[i] = newColSize;
      pyObject2.blockSizes[i] = blockSize;
    }
  
  //Set the actual dataBlocks //ALSO INCLUDE CHECK IF MATRICES IS OF CORRECT SIZES!
  pyObject1.blockData[0] = (char*)M1B1_1;
  pyObject1.blockData[1] = (char*)M1B2_1;
  pyObject1.blockData[3] = (char*)M1B1_2;
  pyObject1.blockData[4] = (char*)M1B2_2;
  pyObject1.blockData[2] = (char*)M1B3_1;
  pyObject1.blockData[5] = (char*)M1B3_2;
  pyObject2.blockData[0] = (char*)M2B1_1;
  pyObject2.blockData[1] = (char*)M2B2_1;
  pyObject2.blockData[2] = (char*)M2B1_2;
  pyObject2.blockData[3] = (char*)M2B2_2;
  pyObject2.blockData[4] = (char*)M2B1_3;
  pyObject2.blockData[5] = (char*)M2B2_3;
  ...
  printf("Done setting metadata on PyObject2.........\n");
  printf("The address of the block[0] on PyObject1 is: %d\n",M1B1_1);
  ...
  //  {
  //    printf(".............Print block %d in PyObject1\n", block);
  //    for(length = 0; length<16;length++)
  //      {
  //        printf("%d",pyObject1.blockData[block][length]);
  //      }
  //    printf("\n");
  //  }
  ...
  //for(block = 0;block<numberOfBlocks;block++)
  //  {
  //    printf(".............Print block %d in PyObject2\n", block);
  //    for(length = 0; length<16;length++)
  //      {
  //        printf("%d",pyObject2.blockData[block][length]);
  //      }
  //    printf("\n");
...
void DivideJobsBetweenSPEs()
{

  //THIS IS INSERTED TO TEST DMA INITIATED FROM SHADER
  test_struct *testStruct = _malloc_align(16,4);
  testStruct->intA = 56;
  testStruct->intB = 66;
  printf("Beginning of DivideJobsBetweenSPE's\n");
  //This function sets job structures
  //(if there's fewer jobs than created SPE job structures,
  //take care not to start more SPE's than there's jobs for)
  
  //Well, actually, we probably just need to send the SPE' an ID.
  //Then they can make a statically orchestrated split themselves.
  
  //JMI:08/03/09 - This is static for now with four blocks pr. matrix.
  
  speJobs[0].SPE_id = 1;
  printf("111\n");
  speJobs[0].howManySPEsAreUsed = 4;
  speJobs[0].pyObject1 = &pyObject1;
  speJobs[0].pyObject2 = &pyObject2;
  speJobs[0].resultObject = &resultObject;
  speJobs[0].functionPointer = main2;
  speJobs[0].shaderSize = 16;
  speJobs[0].EA = testStruct;
  speJobs[1].SPE_id = 2;
  speJobs[1].howManySPEsAreUsed = 4;
  speJobs[1].pyObject1 = &pyObject1;
  speJobs[1].pyObject2 = &pyObject2;
  speJobs[1].resultObject = &resultObject;
  speJobs[1].functionPointer = main2;
  speJobs[1].shaderSize = 16;
  speJobs[1].EA = testStruct;
  speJobs[2].SPE_id = 3;
  speJobs[2].howManySPEsAreUsed = 4;
  speJobs[2].pyObject1 = &pyObject1;
  speJobs[2].pyObject2 = &pyObject2;
  speJobs[2].resultObject = &resultObject;
  speJobs[2].functionPointer = main2;
  speJobs[2].shaderSize = 16;
  speJobs[2].EA = testStruct;
  speJobs[3].SPE_id = 4;
  speJobs[3].howManySPEsAreUsed = 4;
  speJobs[3].pyObject1 = &pyObject1;
  speJobs[3].pyObject2 = &pyObject2;
  speJobs[3].resultObject = &resultObject;
  speJobs[3].functionPointer = main2;
  speJobs[3].shaderSize = 16;
  speJobs[3].EA = testStruct;

  printf("222\n");
  
  for(y = 0;y<4;y++)
    {
      pointersToSpeJobs[y] = &speJobs[y];
      printf("333\n");
    }
  printf("444");
}
//THEN: Test the bastard!

void *ppu_pthread_function(void *arg) {
  spe_context_ptr_t ctx;
  unsigned int entry = SPE_DEFAULT_ENTRY;
 
  pThreadStruct p_thread_struct = *((pThreadStruct*)arg);
 
  //ctx = *((spe_context_ptr_t *)arg);
  ctx = *p_thread_struct.contextPtr;
  ...
  //For runflags = SPE_RUN_USER_REG
 
  //test
  //printf("Adr. of testPointer is %d\n", testPointer);
  //printf("Adr. of M1B1_1: %d\n",M1B1_1);
  //printf("testPointer points to %d\n", *testPointer);
  //printf("M1B1_1[0] points to: %d\n",M1B1_1[0]);
  //printf("M1B1_1[3] points to: %d\n",M1B1_1[3]);
  //printf("p_thread_struct.spe_id is: %d\n",p_thread_struct.spe_id);

  printf("PPE > Adr. of &pointersToSpeJobs[p_thread_struct.spe_id]:\n",&pointersToSpeJobs[p_thread_struct.spe_id]);
  printf("*pyObject1, *pyObject2, *resultObject: %d, %d, %d\n",speJobs[0].pyObject1, speJobs[0].pyObject2, speJobs[0].resultObject);
  if (spe_context_run(ctx,&entry, SPE_RUN_USER_REGS, &pointersToSpeJobs[p_thread_struct.spe_id], NULL, NULL) < 0) {
...
static PyObject *
array_multiply_custom(PyArrayObject2 *m1, PyObject *m2)
{
  
  printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
  fillExampleMatrices();
  
  DivideJobsBetweenSPEs();
  
  //Assuming m2 is a matrix for now.
  
  //Note, the array must be allocated on the heap of course.
  PyObject *resultArray = (PyArrayObject*)_malloc_align(32,4);//How much room are really needed?
  
  //Initialize the resultArray and set appriate values regarding metadata.
  initializeResultMatrix();
  
  //printf("//////////////////////////Adr. resultObject.blockData[0] is: %d\n",(unsigned int)resultObject.blockData[0]);
  
  
  //If some medadata is not set on the two argument arrays, set them.
  
  //Check selected metadata on the result and two argument arrays.
  printf("\n\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n");
  printf("\n\nARRAY_MULTIPLY_CUSTOM: Checking medatadata\n\n");
  
  
  //Start 4 threads on 4 SPE's and upload an SPE image.
  //This SPE image will get an address where it can fetch a metadata structure.
  //In this metadata structure it will say where to find the shader and
  //what argument to give the shader when calling.
  
  //JMI:26/03/09 - These structures are used to signal the finaldispatcher

  unsigned int useTheNewDispatcher = 1;

  if(!useTheNewDispatcher)
    {
#define MAX_SPU_THREADS 6
      spe_program_handle_t *spe_program_handle_t_pointer;
      spe_program_handle_t_pointer = spe_image_open ("/home/jens/final_dispatcher");
      ...
      if (spu_threads > MAX_SPU_THREADS) spu_threads = 6;//MAX_SPU_THREADS;
      /* Create several SPE-threads to execute ’simple_spu’ */
      for(i=0; i<spu_threads; i++) {
	printf("spu_threads is: %d\n",spu_threads);
	/* Create context */
	...
	if (spe_program_load (ctxs[i],spe_program_handle_t_pointer)) {
	  perror ("Failed loading program");
	  exit (1);
	}
	//We keep this in main, since we only leave the function when everything is complete.
	pThreadStruct p_thread_struct;
	p_thread_struct.spe_id = i;
	p_thread_struct.contextPtr = &ctxs[i];
	
	//JMI: 19/03/09 - The structure SPE_JOB_INFO must be expanded to also hold the address of the shader to fetch.
	//This must, of course, be changed, when we have decided how we should handle the stuff on the
	//SPE> this means that we should be able to get shaders and start them with parameters,
	//regardless of whether the existing dispatcher is used or not, in a uniform way.
	//This is what must happen for now:
	//1. An SPU image is uploaded - this will function as the intermediate dispatcher.
	//2. The image gets the address of the SPE_JOB_INFO as a parameter (this is done when the pthread is started).
	//3. The struct has the same info as before, but now also the address of the shader.
	//4. The dispatcher fetches the shader and calls it's main function with the SPE_JOB_INFO struct
	//   as parameter.
	
	/* Create thread for each SPE context */
	if (pthread_create (&threads[i], NULL,&ppu_pthread_function,&p_thread_struct/*&ctxs[i]*/)) {
      ...
      for (i=0; i<spu_threads; i++) {
	unsigned int test = (pthread_join (threads[i], NULL));
	printf("test is: %d\n",test);
	if (test) {
	  ...
	  {
	    printf("Whoooopppy!\n");
	    printf("block1: %d\n",resultObject.blockData[0][0]);
	    printf("block2: %d\n",resultObject.blockData[1][0]);
	    printf("block3: %d\n",resultObject.blockData[2][0]);
	    printf("block4: %d\n",resultObject.blockData[3][0]);
	    printf("R1_1: %d\n",R1_1[0]);
	    printf("R1_1: %d\n",R1_2[0]);
	    printf("R1_1: %d\n",R2_1[0]);
	    printf("R1_1: %d\n",R2_2[0]);
      ...
      //Well, maybe we only need this SPE_IMAGE_LOAD yes?
      //spe_program_handle_t *spe_program_handle_t_pointer;
      //spe_context_ptr_t speid;
      //unsigned int entry=SPE_DEFAULT_ENTRY;
      
      //spe_program_handle_t_pointer = spe_image_open ("/home/jens/spu_test");
      
      //speid = spe_context_create(0,NULL);
      //spe_program_load(speid,spe_program_handle_t_pointer);
      //spe_context_run(speid,&entry,0 ,NULL ,NULL ,NULL);
    }
  else
    {
      printf("Before calling main_from_PPUTEST()\n");
      main_from_PPUTEST();
      //return resultArray;
      resultArray = (PyArrayObject*)&resultObject;
    }
  printf("\n\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!END!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n");
  ...
  //Make data available for the SPE's to get.
  
  //For now! Upload an SPE image that DMA's the shader to the SPE's
  
  
  //"Wait" for the SPE's to write their results back.
  
  //Return a pointer to the result array.
  //printf("Values of block[0]: %d, %d, %d, %d, %d\n",((PyArrayObject*)resultArray)->blockData[0][0],((PyArrayObject*)resultArray)->blockData[0][1],((PyArrayObject*)resultArray)->blockData[0][2],((PyArrayObject*)resultArray)->blockData[0][3],((PyArrayObject*)resultArray)->blockData[0][4]);
...
//JMI end.............................................................................
  ...
  static PyObject *
    ...
    {
      printf("-*---*----*----*------*------*--------*-------*-------------ARRAYOBJECT mult\n");
  printf("------------------------------------------5555555555------------------------------------------------------------\n");

  //JMI 17/03/09 - For now we insert the multiplication function here.
  //A pointer to the result array is returned.
  
  //JMI 17/03/09 - vi bliver lige noedt til at tjekke det objekt der kommer tilbage fra generic-funktionen,
  //for at se hvorledes vi skal saette data saa alt gaar godt.
  //Andre issues: Laver generic funktionen andet end at saette PyObject'et, som er absolut noedvendigt for
  //opnaaelse af success??

  //Create an enum somewhere:
  //0:add
  //1:subtract
  //2:multiply
  //...
  //How do we distinguish between pair-wise/non-pair-wise and BLASX?
  //Fow now let's assume we do it in the shaderwitcher.

  //PyObject *resultObject;//JMI: Put this back in after multiply test... 
  //GenericShaderSwitcher(2,m1,m2);

  //PyObject *resultObject = array_multiply_custom(m1, m2);
  

  //JMI: 20/03/09 - This is what it looked like before
  //printf("OKAAAAAY, n_ops.multiply is: %d, %d, %d, %d\n",((char*)n_ops.multiply)[0],((char*)n_ops.multiply)[1],((char*)n_ops.multiply)[2],((char*)n_ops.multiply)[3]);

  //JMI: Print out info about the arguments

  printf("Argument 1\n");
  printf("m1->data[0]: %d\n",((unsigned int*)m1->data)[0]);
  printf("m1->data[0]: %d\n",((unsigned int*)m1->data)[1]);
  printf("m1->dimensions[0]: %d\n",m1->dimensions[0]);
  printf("m1->nd: %d\n",m1->nd);
  printf("m1->slice: %d\n",m1->slice_type);

  printf("Argument 2\n");
  printf("((PyArrayObject*)m2)->data[0]: %d\n",((unsigned int*)((PyArrayObject*)m2)->data)[0]);
  printf("((PyArrayObject*)m2)->data[0]: %d\n",((unsigned int*)((PyArrayObject*)m2)->data)[1]);
  printf("((PyArrayObject*)m2)->dimensions[0]: %d\n",((PyArrayObject*)m2)->dimensions[0]);
  printf("((PyArrayObject*)m2)->nd: %d\n",((PyArrayObject*)m2)->nd);


  PyObject *resultObject;
  //PyObject *resultObject = PyArray_GenericBinaryFunction(m1, m2, n_ops.multiply);
  printf("11111AFTER WE HAVE RETURNED FROM CALL TO ARRAY_MULTIPLY_CUSTOM11111111\n");

  //Print medadata from the resultObject:
  ...
  //PyArrayObject2 *array;
  //array = (PyArrayObject2*)resultObject;

  //printf("resultObject->BlockData[0][1] and 3 more... is: %d, %d, %d, %d\n",array->blockData[0][0],array->blockData[0][1],array->blockData[0][2],array->blockData[0][3]);

  printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
  //printf("Number of dimensions: %d\n",array->nd);
  //printf("numberOfBlocksXDim: %d\n",array->numberOfBlocksXDim);
  //printf("numberOfBlocksYDim: %d\n",array->numberOfBlocksYDim);
  //printf("flags: %d\n",array->flags);
  //printf("numberOfBlocks: %d\n",array->numberOfBlocks);
  //printf("Addresses of block[0] and block[0][0]: %d, %d\n",array->blockData[0],&array->blockData[0][0]);
  //printf("Values of block[0]: %d, %d, %d, %d, %d\n",array->blockData[0][0],array->blockData[0][1],array->blockData[0][2],array->blockData[0][3],array->blockData[0][4]);
  //printf("Number of dimensions: %d\n",array->nd);
  //printf("Number of dimensions: %d\n",array->nd);
  //printf("Number of dimensions: %d\n",array->nd);
  //printf("Number of dimensions: %d\n",array->nd);
...
//JMI:26/03/09 - Here I insert the PPU calls regarding the new dispatcher
//Note that stuff is included for this in the directives at the
...
unsigned int spe_pointer_addr[MAX_SPE_THREADS][6] __attribute__((aligned(16)));
PpuPthreadData_t speData[MAX_SPE_THREADS];

unsigned int speThreads;
*/

int main_from_PPUTEST(/* int argc, char *argv[]*/ )
{
  ...
  */
  ...
  shader = main2;
  printf( "Calling MULT();\n" );
  //MULT( &pyObject1, &pyObject2, &resultObject, MAIN_SIZE, shader );
...
array_slice(PyArrayObject *self, Py_ssize_t ilow,
            Py_ssize_t ihigh)
{
  printf("arrayobject::array_slice\n");
    PyArrayObject *r;
    Py_ssize_t l;
    char *data;
...
array_ass_slice(PyArrayObject *self, Py_ssize_t ilow,
  ...
  printf("arrayobject::array_ass_slice\n");
    int ret;
    PyArrayObject *tmp;
...
{
  printf("arrayobject::array_contains\n");
...
{
  printf("arrayobject.c5367callfunction\n");
...
{
  //JMI: This function is called by PyArray_Newshape which is called by ravel.
  printf("arrayobject.c\n");
  printf("arrayobject::PyArray_NewFromDescr number %d..................................\n",i++);
  if(!(data == NULL))
    {
      printf("data is................");
      printf("Data char %d\n",*(((unsigned int *)data)));
  ...
  size_t sd;
    ...
    if (descr->subarray) {
        ...
        isfortran = (data && (flags & FORTRAN) && !(flags & CONTIGUOUS)) || \
            (!data && flags);
	printf("memcpy(newdims, dims, nd*sizeof(intp)); 4...........................\n");
        memcpy(newdims, dims, nd*sizeof(intp));
        if (strides) {
            newstrides = newdims + MAX_DIMS;
	    printf("memcpy(newdims, dims, nd*sizeof(intp)); 5.....................................\n");
	...
	printf("memcpy(self->dimensions, dims, sizeof(intp)*nd); 1............................\n");
        memcpy(self->dimensions, dims, sizeof(intp)*nd);
	//JMI: So how does the memory look after this call?
	if(!(data == NULL))
	  {
	    printf("Data char %d\n",*(((unsigned int *)data)));
	    printf("Data char %d\n",*(((unsigned int *)data+1)));
	  ...
	  {
	    printf("NewFromDescription : Data was NULL.........................");
	  }
        ...
        if (strides == NULL) { /* fill it in */
            sd = _array_fill_strides(self->strides, dims, nd, sd,
               ...
               */
	  printf("memcpy(self->strides, strides, sizeof(intp)*nd); 2...............................\n");
    ...
    if(!(data == NULL))
      {
	printf("NewFromDescription after self->data = data;... Data char 0 %d\n",((char*)data)[0]);
	printf("NewFromDescription after self->data = data;... Data char 1 %d\n",((char*)data)[1]);
	printf("NewFromDescription after self->data = data;... Data char 2 %d\n",((char*)data)[2]);
	printf("NewFromDescription after self->data = data;... Data char 3 %d\n",((char*)data)[3]);
	printf("NewFromDescription after self->data = data;... Data char 4 %d\n",((char*)data)[4]);
        printf("NewFromDescription after self->data = data;... Data char 5 %d\n",((char*)data)[5]);
        printf("NewFromDescription after self->data = data;... Data char 6 %d\n",((char*)data)[6]);
        printf("NewFromDescription after self->data = data;... Data char 7 %d\n",((char*)data)[7]);
	printf("NewFromDescription after self->data = data;... Data char 8 %d\n",((char*)data)[8]);
        printf("NewFromDescription after self->data = data;... Data char 9 %d\n",((char*)data)[9]);
        printf("NewFromDescription after self->data = data;... Data char 10 %d\n",((char*)data)[10]);
        printf("NewFromDescription after self->data = data;... Data char 11 %d\n",((char*)data)[11]);
    ...
    */
    ...
    //JMI: Can be catch the data here?
    printf("End of NewFromDescr..............\n");
    printf("self->data = data;... Data char 0 %d\n",((PyArrayObject*)self)->data[0]);
    printf("self->data = data;... Data char 1 %d\n",((PyArrayObject*)self)->data[1]);
    printf("self->data = data;... Data char 2 %d\n",((PyArrayObject*)self)->data[2]);
    printf("self->data = data;... Data char 3 %d\n",((PyArrayObject*)self)->data[3]);
    printf("self->data = data;... Data char 4 %d\n",((PyArrayObject*)self)->data[4]);
    printf("self->data = data;... Data char 5 %d\n",((PyArrayObject*)self)->data[5]);
    printf("self->data = data;... Data char 6 %d\n",((PyArrayObject*)self)->data[6]);
    printf("self->data = data;... Data char 7 %d\n",((PyArrayObject*)self)->data[7]);
    printf("self->data = data;... Data char 8 %d\n",((PyArrayObject*)self)->data[8]);
    printf("self->data = data;... Data char 9 %d\n",((PyArrayObject*)self)->data[9]);
    printf("self->data = data;... Data char 10 %d\n",((PyArrayObject*)self)->data[10]);
    printf("self->data = data;... Data char 11 %d\n",((PyArrayObject*)self)->data[11]);
...
{
  printf("arretertvervtayobject.c\n");
    Py_ssize_t i, slen;
    int res = 0;
...
{
  printf("arrayobject.callfunctionc\n");
    PyObject *new;
    PyObject *array_meth;
...
{
  printf("arrayobject::iter_subscript\n");
...
{
  printf("arrayobject::iter_ass_subscript\n");
...
{
  printf("arrayobject.c\n");
  printf("arrayobject::_convert_obj\n");
