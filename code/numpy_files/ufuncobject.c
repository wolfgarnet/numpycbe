...
_has_reflected_op(PyObject *op, char *name)
{
  ...
  printf("_has_reflected_op(PyObject *op, char *name)\n");
    _GETATTR_(add, radd);
...
construct_arrays(PyUFuncLoopObject *loop, PyObject *args, PyArrayObject **mps,
                 PyObject *typetup)
{
	printf("ufuncobject::construct_arrays\n");
    Py_ssize_t nargs;
    int i;
...
{
  printf("GENERIC UFUNC CALL callfunction55\n");
    int i;
    PyTupleObject *ret;
    ...
    for(i = 0; i < self->nout; i++) {
	    ...
	    printf("callycally\n");
            ...
            if (res == NULL && \
                ...
                PyErr_Clear();
		printf("here there is a callfunction call...\n");
    ...
    if (self->nout == 1) {
      /*
      printf("returning...\n");
      printf("Well, let's check this bastard!\n");
      printf("unsigned int 1: %d\n", ((PyArrayObject*)retobj[0])[0]);
      printf("unsigned int 2: %d\n", ((PyArrayObject*)retobj[0])[1]);
      printf("unsigned int 3: %d\n", *((char*)((unsigned int*)retobj[0])[2]));
      printf("unsigned int 4: %d\n", ((unsigned int*)retobj[0])[3]);
      printf("unsigned int 5: %d\n", *((char*)((unsigned int*)retobj[0])[4]));
      printf("unsigned int 6: %d\n", *((char*)((unsigned int*)retobj[0])[5]));
      printf("unsigned int 7: %d\n", ((PyArrayObject*)retobj[0])[6]);
      printf("unsigned int 8: %d\n", ((PyArrayObject*)retobj[0])[7]);
      printf("unsigned int 9: %d\n", ((unsigned int*)retobj[0])[8]);
      printf("unsigned int 10: %d\n", ((unsigned int*)retobj[0])[9]);
      printf("unsigned int 11: %d\n", ((unsigned int*)retobj[0])[10]);
      printf("unsigned int 12: %d\n", ((unsigned int*)retobj[0])[11]);
      */
	...
	printf("returnCheck this PyObject out and see what data it has!!!\n");
        return (PyObject *)ret;
    }
fail:
    for(i = self->nin; i < self->nargs; i++) {
