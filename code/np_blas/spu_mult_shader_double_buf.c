...
{
	int *LS_DATA_AREA_1_Ptr;
	int *LS_DATA_AREA_2_Ptr;
	int *LS_DATA_AREA_3_Ptr;
	int *LS_DATA_AREA_4_Ptr;
	int *LS_DATA_AREA_5_Ptr;
	int *LS_DATA_AREA_6_Ptr;
	int padding[1];
...
void doCalculationsWithinTheBlocks(LS_ShaderInfo *info, unsigned int index, Functions_t *funcs, int *resPointer)
{
	typedef unsigned int (PrintUInt)(unsigned int);
  	...
  	if(index==0)
	{
  			...
  			{	
  				...
  				for(innerColCount = 0; innerColCount < blockColSize;innerColCount++)
  				{
  					...
  					{
  						//It's okay to use the same pointer to resultdata here, since this code will not run overlapped.
  						//Tip: We could make a second function to avoid this check everytime!
  						
  						resPointer[actualIndex3] = resPointer[actualIndex3] + ((METADATA*)info->LS_shaderMemory)->LS_DATA_AREA_1_Ptr[actualIndex1]*((METADATA*)info->LS_shaderMemory)->LS_DATA_AREA_2_Ptr[actualIndex2];
  			...
  			{	
  				...
  				for(innerColCount = 0; innerColCount < blockColSize;innerColCount++)
  				{
  					...
  					for(innerInnerCounter = 0; innerInnerCounter < blockColSize; innerInnerCounter++)
  					{
  						...
  						resPointer[actualIndex3] = resPointer[actualIndex3] + ((METADATA*)info->LS_shaderMemory)->LS_DATA_AREA_4_Ptr[actualIndex1]*((METADATA*)info->LS_shaderMemory)->LS_DATA_AREA_5_Ptr[actualIndex2];
...
void doTheMultThingy(unsigned int SPE_id, LS_ShaderInfo *info, Operation_t *myop, Functions_t *funcs)
{
	...
	int *bufPointer1 = ((METADATA*)info->LS_shaderMemory)->LS_DATA_AREA_1_Ptr;
	int *bufPointer2 = ((METADATA*)info->LS_shaderMemory)->LS_DATA_AREA_2_Ptr;
		
	int *resPointer = ((METADATA*)info->LS_shaderMemory)->LS_DATA_AREA_3_Ptr;
	...
	{	
		...
		for(outerLoopCounter = 1; outerLoopCounter < (((METADATA*)info->LS_shaderMemory)->pyArrayObject[0].numberOfBlocksXDim  );outerLoopCounter++)
		{
			...
			doCalculationsWithinTheBlocks(info,zero_one,funcs, resPointer);
